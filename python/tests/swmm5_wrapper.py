...
from ctypes import c_double, CDLL, c_void_p, c_float, pointer, c_char_p, c_int, POINTER # Required to handle with DLL variable
import os
...
class SWMM5:
	def __init__(self):
		self._SWMM5 = CDLL(os.path.join(os.getcwd(),libpath)) # C library
		self._SWMM5.api_initialize.restype = c_void_p
		self._SWMM5.api_finalize.argtypes = (c_void_p,)
		self._SWMM5.api_get_node_results.argtypes = (c_void_p, c_char_p, c_float_p, c_float_p, c_float_p, c_float_p,)
		self._SWMM5.api_get_link_results.argtypes = (c_void_p, c_char_p, c_float_p, c_float_p, c_float_p,)
		self._SWMM5.api_get_node_attribute.argtypes = (c_void_p, c_int, c_int, c_double_p,)
		self._SWMM5.api_get_link_attribute.argtypes = (c_void_p, c_int, c_int, c_double_p,)
		self._SWMM5.api_get_num_objects.argtypes = (c_void_p, c_int,)
		self._SWMM5.api_export_linknode_properties.argtypes = (c_void_p, c_int,)
		self._SWMM5.api_export_link_results.argtypes = (c_void_p, c_char_p,)
		self._SWMM5.api_export_node_results.argtypes = (c_void_p, c_char_p,)
		self._api = c_void_p()
		self._elapsedTime = c_double(0.000001) # Elapsed time in decimal days
		self._ptrTime = pointer( self._elapsedTime ) # Pointer to elapsed time
		self._start_time = time() # Simulation start time
		self._end_time = time() # Simulation end time
	...
	def initialize(self, inp):
		...
		self._api = self._SWMM5.api_initialize(c_char_p(six.b(inp)), c_char_p(six.b(rpt)), c_char_p(six.b(out))) # Step 1

	def finalize(self):
		self._SWMM5.api_finalize(self._api)
	...
	def run_step(self):
		...
		error = self._SWMM5.swmm_step(_ptrTime)
		if (error != 0):
			raise SystemError ("Error %d ocurred at time %.2f" % (error, self._elapsedTime.value))
		return self._elapsedTime.value
	...
	def is_over(self):
		...
		'''
		return self._elapsedTime.value == 0.0
	...
	def get_time(self):
		...
		'''
		return self._elapsedTime.value*24

	def save_report(self, msg=False):
	...
	def get_mass_bal_error(self):
		...
		error = self._SWMM5.swmm_getMassBalErr(ptrRunoff, ptrFlow, ptrQual)
		if (error != 0):
	...
	def get_node_results(self, nodes):
		inflows = []
		overflows = []
		depths = []
		volumes = []
		...
		for node in nodes:
			...
			self._SWMM5.api_get_node_results(
				self._api,
				c_char_p(six.b(node)),
				ptr_inflow,
				ptr_overflow,
	...
	def get_link_results(self, links):
		flows = []
		depths = []
		volumes = []
		...
		for link in links:
			...
			ptr_depth = pointer(depth)
			ptr_volume = pointer(volume)
			self._SWMM5.api_get_link_results(
				_api,
				c_char_p(six.b(link)),
				ptr_flow,
				ptr_depth,
	...
	def get_node_attribute(self, nodes, attr):
		...
		for i, node in enumerate(nodes):
			...
			error = self._SWMM5.api_get_node_attribute(self._api, j, attr, ptr_value)
			if error != 0:
				raise Exception(f"ERROR CODE [{error}]")
			values[i] = value.value
		return values

	def get_link_attribute(self, links, attr):
		values = np.zeros(len(links))
		value = c_double(0.0) # Elapsed time in decimal days
		ptr_value = pointer( value )
		for i, link in enumerate(links):
			j = find_object(LINK, link)
			error = self._SWMM5.api_get_link_attribute(self._api, j, attr, ptr_value)
			if error != 0:
				raise Exception(f"ERROR CODE [{error}]")
			values[i] = value.value
		return values

	def get_num_objects(self, obj_type):
		return self._SWMM5.api_get_num_objects(self._api, obj_type)

	# --- Print-out

	def export_linknode_properties(self, inp, units):
		...
		error = self._SWMM5.api_export_linknode_properties(self._api, units)
	...
	def export_link_results(self, link_id):
		self._SWMM5.interface_save_link_results(self._api, c_char_p(six.b(link_id)))

	def export_node_results(self, node_id):
		self._SWMM5.interface_save_node_results(self._api, c_char_p(six.b(node_id)))

	# --- Tests

	def find_object(self, obj_type, obj_name):
		return self._SWMM5.api_findObject(obj_type, c_char_p(six.b(obj_name)))

	def print_inflow(self, node_id):
		self._SWMM5.interface_print_inflow(c_char_p(six.b(node_id)))

	def print_tseries_props(self, k):
		self._SWMM5.interface_print_tseries_props(k)